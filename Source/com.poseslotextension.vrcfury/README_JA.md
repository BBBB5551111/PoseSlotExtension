...
3. **v1.x を導入済みの場合は、先に `Assets/PoseSlotExtensionVRCFuryStable` フォルダを削除します。** v2.0で導入先が `Packages/` へ変わったため、両方が残るとクラスが重複します。
4. 本UnityPackageをImportします。`Packages/com.poseslotextension.vrcfury` へ展開され、Unityの Package Manager に `Pose Slot Extension (VRCFury)` として表示されます。`Assets/` には何も追加されません。
5. `Tools > Pose Slot Extension VRCFury > Setup`を開きます。
...
アンインストールは `Packages/com.poseslotextension.vrcfury` フォルダを削除するだけです。

## 非破壊方針

- BUDDYWORKS Package原本は編集しません。
- Action ControllerとExpression Menuを`Packages/com.poseslotextension.vrcfury/Generated`へ複製して加工します。
...
実プロジェクトへ生成・導入し、VRCFury＋NDMF後の50 Save、50 Load、151個のPSEパラメータ、空スロット、上書き、高速連続Load、ポーズ保持、Reset、最上段配置、8項目上限を検証しています。さらにVRChat公式SDKのBuildと実機動作を確認済みです。特定のアバター名、階層名、メッシュまたは衣装には依存しません。
