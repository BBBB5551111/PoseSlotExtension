...
namespace PoseSlotExtensionVRCFuryStable.Editor
{
    public static class PoseSlotExtensionGenerator
    {
        ...
        private const string Root = "Packages/com.poseslotextension.vrcfury";
        ...
        [MenuItem("Tools/Pose Slot Extension VRCFury/Generate 50 Save Load Slots")]
        public static void Generate()
        {
            try
            {
                ...
                if (!Silent) EditorUtility.DisplayDialog("Pose Slot Extension", "Save 01-50 / Load 01-50 を生成しました。", "OK");
            }
        ...
        {
            // This stable line targets the [VRCF] prefab; the installer also
            // auto-places it, so its presence is the required package signal.
        ...
        private static AnimatorController GenerateController()
        {
            ...
            {
                AddParameter(controller, SnapshotSet(slot), AnimatorControllerParameterType.Int, 0);
                AddParameter(controller, SnapshotPose(slot), AnimatorControllerParameterType.Float, 0);
                AddParameter(controller, SnapshotValid(slot), AnimatorControllerParameterType.Bool, 0);
            }
        ...
        private static void BuildCommandLayer(AnimatorController controller)
        {
            ...
            for (var slot = 1; slot <= PoseSlotFixedSpecification.SlotCount; slot++)
            {
                ...
                var saveTransition = machine.AddAnyStateTransition(save);
                saveTransition.canTransitionToSelf = false;
                ...
                AddDriver(save,
                    ...
                    Set(SnapshotValid(slot), 1));
                // Leave as soon as PSE/Command is anything other than this slot's own
                // command. The previous condition (Equals 0) meant a command arriving
                // before the menu had sent 0 was swallowed: the state machine could not
                // leave the current state, so the next command was silently dropped.
                // VRChat's expression menu emits a button's value as the cursor passes
                // over it, so sweeping across neighbouring Save/Load buttons either did
                // nothing or wrote the neighbouring slot instead of the intended one.
                var saveReturn = save.AddTransition(idle);
                AddCondition(saveReturn, AnimatorConditionMode.NotEqual,
                    PoseSlotFixedSpecification.SaveCommand(slot), Command);

                var load = AddState(machine, $"PSE_Load_{slot:00}", new Vector3(700, slot * 45));
                var loadTransition = machine.AddAnyStateTransition(load);
                loadTransition.canTransitionToSelf = false;
                AddCondition(loadTransition, AnimatorConditionMode.Equals, PoseSlotFixedSpecification.LoadCommand(slot), Command);
                AddCondition(loadTransition, AnimatorConditionMode.If, 0, SnapshotValid(slot));
                AddDriver(load,
                ...
                AddCondition(loadReturn, AnimatorConditionMode.NotEqual,
                    PoseSlotFixedSpecification.LoadCommand(slot), Command);
            }
        }
        ...
        {
            var root = new GameObject("PoseSlotExtensionVRCFuryStable");
            try
            {
        ...
        {
            // PSE/Command is supplied once by the VRCFury parameters asset because
            // the menu itself is installed through VRCFury. Do not also declare it
            // here; duplicate declarations caused the SDK menu validation failure.
            var result = new List<ParameterConfig>();
            for (var slot = 1; slot <= PoseSlotFixedSpecification.SlotCount; slot++)
            {
                result.Add(Config(SnapshotSet(slot), ParameterSyncType.Int, true, true, 0));
                result.Add(Config(SnapshotPose(slot), ParameterSyncType.Float, true, true, 0));
                result.Add(Config(SnapshotValid(slot), ParameterSyncType.Bool, true, true, 0));
            }
            return result;
        }
        ...
        private static void WriteDocumentation(AnimatorController controller, VRCExpressionsMenu menu)
        {
            ...
            // README_JA.md is a curated distribution document; regeneration must
            // not overwrite it inside user projects.
        }
        ...
        private static VRCAvatarParameterDriver.Parameter Set(string destination, float value) =>
            new VRCAvatarParameterDriver.Parameter { name = destination, value = value, type = VRCAvatarParameterDriver.ChangeType.Set };
        ...
        private static string SnapshotValid(int slot) => PoseSlotFixedSpecification.SnapshotValid(slot);
    }
}
#endif
